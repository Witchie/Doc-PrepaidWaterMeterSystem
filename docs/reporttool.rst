...
水平和垂直标尺以报表指定的度量单位显示刻度线。单击某个元素以使用标尺计算其大小和位置。
垂直标尺还允许您通过移动顶部和底部滑块来更改标注栏高度。
...
在报表设计器中，每个报表栏都带有标题、选项卡标题和颜色，这取决于标注栏的类型。这些标题不会打印在生成的报表文档中，仅在设计时可见。

.. figure:: img/r9.png

要访问标注栏属性，请通过单击标注栏标题来选择标注栏，然后切换到“属性”面板。

右键菜单
^^^^^^^^^^^^^

上下文菜单提供了对最常用操作的快速访问，这些操作取决于调用它的元素。例如，它允许您插入新的带区、剪切/复制/粘贴/删除报表控件等。对于某些报表控件，上下文菜单还包括编辑。。。选项，它将调用具有复杂设置（如数据绑定设置、格式设置等）的对话框，若要调用此菜单，请右键单击报表元素或报表编辑图面。
...
大多数报表元素（标注栏和控件）都提供包含最流行的上下文相关设置的智能标记。您可以通过选择所需元素并单击智能标记按钮（带有右箭头图标）来调用智能标记。
...
In-place编辑器允许您通过双击面向文本的控件（XRBarCode、XRCheckBox、XRLabel、XRTableCell或XRZipCode）来编辑其内容。
...
脚本编辑器允许最终用户为最终用户报表设计器中的特定事件处理程序编写代码，以调整报表控件、带区或报表本身的行为。

.. figure:: img/r1.png

本主题介绍使用脚本的基本知识、脚本编辑器界面，并说明如何在报表中使用脚本

脚本预览
^^^^^^^^^^^^^^^

脚本编辑器为最终用户提供了在生成报表时在运行时编写和执行脚本的功能。请注意，此功能主要用于希望在最终用户报表设计器中稍微自定义报表的高级最终用户。
脚本编辑器支持C#和Visual Basic.NET脚本语言。这意味着脚本语言独立于用于创建报表的语言。可以使用指定语言XtraReport.ScriptLanguage财产。对于报表中使用的所有脚本，所选脚本语言应相同。
...
每个元素都有自己的单独的事件集。要处理报表元素的事件，请执行以下操作之一。

    * 在设计图面上选择所需的报表元素。在“属性”面板中，展开“脚本”属性并单击事件的加号按钮。
    
    .. figure:: img/r3.png

    * 单击工具栏中的“脚本”按钮（WPFDesigner_Toolbar_ScriptEditor）以显示脚本编辑器。在脚本编辑器左上角的专用下拉列表中选择所需的报表元素。然后，在右上角的另一个列表中选择一个可用事件。

    .. figure:: img/r4.png

指定事件后，将自动以当前脚本语言生成代码模板，并将其添加到脚本编辑器中

.. figure:: img/r5.png

要检查报表脚本中的错误，请单击“验证”按钮。验证结果显示在脚本编辑器底部的“错误”面板中。双击面板列表中的错误项以转到相应的代码行。如果所有脚本都有效，则“错误”面板为空。
