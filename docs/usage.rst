...
超级报表可以依据 要统计的开始日期、结束日期以及统计类型将用户的消费数据做一个统计报表来。

效果如下图

.. figure:: img/super1.png

.. important::
      - 特别提醒，这里算出来的基本水费是以第一阶梯单价计算出来的。所算数据和用户是有出入的。由于预付费的原因系统是无法知道用户的实际用水信息的，此处计算数据为猜测信息，仅供参考


超级报表1
~~~~~~~~~~~~~~~

超级报表1可以依据 要统计的开始日期、结束日期以及统计类型将用户的消费数据做一个统计报表来。与超级报表不同的是，这里可以统计水资源费用

.. figure:: img/super2.png

.. note::
      - 同超级报表，这里的水资源费信息和污水管理费也是使用 第一阶梯单价计算出来的。数据仅供参考，并非实际信息。
